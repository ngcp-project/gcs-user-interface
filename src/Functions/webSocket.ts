...
const wsConnections: { [key: string]: WebSocket } = {}; // dictionary where vehicle name is the key (eru, fra, etc), and its websocket connection is the value
...
export function initializeWSConnections() {
  for (const [vehicleKey, connectionURL] of Object.entries(connections)) {
    const newWebSocket = new WebSocket(connectionURL);
...
export function closeConnections() {
  for (const vehicleKey in wsConnections) {
